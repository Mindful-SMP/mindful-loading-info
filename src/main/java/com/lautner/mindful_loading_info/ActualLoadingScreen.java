...
public class ActualLoadingScreen {
    ...
    public static void startLoadingScreen(boolean fabricReady) {
        ...
        if (IS_HEADLESS) {
            println("Mindful Loading Info is on a headless environment. Only some logging will be performed.");
            return;
        }
    ...
    private static void println(String message, Throwable t) {
        final String prefix = IS_IPC_CLIENT
            ? "[MindfulLoadingInfo (IPC client)] "
            : ENABLE_IPC
                ? "[MindfulLoadingInfo (IPC server)] "
                : "[MindfulLoadingInfo] ";
    ...
    public static void main(String[] args) {
        try {
            ...
            JOptionPane.showMessageDialog(
                dialog, "An error occurred in Mindful Loading Info's IPC client\n" + e,
                "Mindful Loading Info IPC Client", JOptionPane.ERROR_MESSAGE
            );
        }
        close();
    }
